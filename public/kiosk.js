...
function requestTicket(isPriority) {
    ...
    // Prevent double-clicks: disable selection buttons while processing
...
socket.on('ticket_assigned', (data) => {
    showResult(data);
    // Re-enable selection buttons after ticket has been shown
    document.querySelectorAll('#selectionOverlay .btn').forEach(b => b.disabled = false);
});

socket.on('ticket_error', (err) => {
    // Show a simple inline error modal if ticket creation failed
