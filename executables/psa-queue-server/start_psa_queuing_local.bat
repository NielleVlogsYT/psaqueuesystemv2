...
if not exist "server.js" (
    ...
    echo Open this script from the PSAQueue_local folder.
    pause
    exit /b 1
)
...
echo Log file: %~dp0local-server.log

start "PSA Queuing Local Server" /min cmd /c "node server.js > local-server.log 2>&1"
...
if errorlevel 1 (
    ...
    echo %~dp0local-server.log
    echo.
    type local-server.log
    pause
    exit /b 1
)
